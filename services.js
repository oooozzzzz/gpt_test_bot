...
const { getGPTContext, pushToGRPContext, setGPTContext, getMemorySize } = require("./context");
...
module.exports.getGPTAnswerWithContext = async (id, text, command = "") => {
	...
	if (getGPTContext(id).length > getMemorySize() + 1) {
		const context = getGPTContext(id);
		const newContext = context.splice(3, 2);
...
module.exports.getTranscription = async (filePath) => {
	const transcription = await client.audio.transcriptions.create({
		file: fs.createReadStream(filePath),
		model: "whisper-1",
	});
